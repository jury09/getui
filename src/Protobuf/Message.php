...
use getui\Protobuf\Reader\AbstractInputReader;
use getui\Protobuf\Reader\InputStringReader;
...
abstract class Message
{
     ...
     * @param AbstractInputReader $reader
     */
    public function __construct($reader = null)
    {
        $this->reader = $reader;
        $this->value = $this;
        $this->base128 = new Base128varint(Message::MODUS);
    }

    /**
     * Get the wired_type and field_type
     * @param mixed $number as decimal
    ...
    public function SerializeToString($rec = -1)
    {
        ...
        if ($this->wired_type == Message::WIRED_LENGTH_DELIMITED && $rec > -1) {
            $stringinner = $this->base128->set_value(strlen($stringinner) / Message::MODUS) . $stringinner;
        }
    ...
    private function _ParseFromArray($length = 99999999)
    {
        ...
        while ($this->reader->get_pointer() - $_begin < $length) {
            ...
            if (!isset($this->fields[$messtypes['field']])) {
                ...
                if ($messtypes['wired'] == Message::WIRED_LENGTH_DELIMITED) {
                    $consume = new PbString($this->reader);
                } else if ($messtypes['wired'] == Message::WIRED_VARINT) {
    ...
    /**
     * Get array value
     * @param mixed $index id of the field
     * @param mixed $value
     *
     * @return mixed
     */
     ...
     * @param Message $class class where the request should be encoded
     *
