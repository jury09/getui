...
class Http
{
    ...
    private static function httpPost($url, $data, $gzip, $action)
    {
        ...
        curl_setopt($curl, CURLOPT_CONNECTTIMEOUT_MS, Config::getHttpConnectionTimeOut());
        curl_setopt($curl, CURLOPT_TIMEOUT_MS, Config::getHttpSoTimeOut());
        ...
        //请求失败有3次重试机会
        $result = Http::exeBySetTimes(3, $curl);
        //curl_close($curl);
        return $result;
    }
    ...
    public static function httpPostJson($url, $params, $gzip)
    {
        if (!isset($params["version"])) {
            $params["version"] = Config::getSDKVersion();
        }
        $action = $params["action"];
        $data = json_encode($params);
        $result = null;
        try {
            $resp = Http::httpPost($url, $data, $gzip, $action);
    ...
    private static function exeBySetTimes($count, $curl)
    {
        ...
        if (curl_errno($curl) != 0 && $code != 200) {
            Log::debug("request errno: " . curl_errno($curl) . ",url:" . $info["url"]);
            $count--;
            if ($count > 0) {
                $result = Http::exeBySetTimes($count, $curl);
            }
        }
        return $result;
    }
